...
void ReplaceDialog::on_btExchange_clicked()
{
    QString target = ui->searchText->text();
...
void ReplaceDialog::on_btExchangeAll_clicked()
{
    QString target = ui->searchText->text();
    QString to = ui->targetText->text();
    ...
    if((pTextEdit !=nullptr)&&(target !="")&&(to !="")){
         QString text = pTextEdit->toPlainText();
             text.replace(target,to,ui->cb->isChecked() ?Qt::CaseSensitive : Qt::CaseInsensitive);
             pTextEdit->clear();
